...
public class GRAPHs {
    ...
    public boolean isCyclicDfs(ArrayList<ArrayList<Integer>> adj, int i, int child, boolean[] vis, int[] parent) {
        if(vis[i] && parent[i] == child) return true;
        vis[i] = true;
        for(int neighbor : adj.get(i)){
            parent[neighbor] = i;
            if(isCyclicDfs(adj, neighbor, i, vis, parent)) return true;
        }
        return false;
    }

    public boolean isCyclic(ArrayList<ArrayList<Integer>> adj, int i, boolean[] vis, int[] parent) {
        Queue<Integer> q = new LinkedList<>();
        q.offer(i);
        parent[i] = -1;
        vis[i] = true;
